...
def check_weather():
    ...
    city = input(f"{INFO}Enter city name: {RESET}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"{SUCCESS}Weather in {city}:{RESET}")
            print(f"{INFO}Temperature: {temp}°C{RESET}")
            print(f"{INFO}Description: {description.capitalize()}{RESET}")
        else:
            print(f"{ERROR}Failed to retrieve weather data. Status code: {response.status_code}{RESET}")
    except Exception as e:
        print(f"{ERROR}An error occurred: {e}{RESET}")
    pause()
...
def main_menu():
    while True:
        try:
            ...
            print("9. Check Weather")
            print("10. Trivia Quiz")
            print("11. Exit\n")

            choice = input(f"{INFO}Enter your choice (1-11): {RESET}")
